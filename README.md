...
работать — первым делом обновите yt-dlp: запустите **`update.bat`**
(Windows) или **`./update.sh`** (macOS/Linux).
...
| X (Twitter) не качает | Скачиваются только видео из публичных постов; посты 18+ и закрытые аккаунты требуют входа и не поддерживаются |
| Facebook не качает | Только публичные видео; закрытые группы не поддерживаются |
